 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> first_diff = 0.0;
      for (int i = 1; i < coeffs.size(); i++) {
        first_diff += coeffs[i]*CppAD::pow(x0, i-1)*i;
      }
      AD<double> psides0 = CppAD::atan(first_diff);
...
// latency in seconds
//
MPC::MPC(double latency) {
  previous_delta = 0.0;
  previous_a = 0.0;
  n_latency = int(round(latency/dt));
}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  previous_a = solution.x[a_start+n_latency];
  ...
  return {solution.x[x_start + n_latency],   solution.x[y_start + n_latency],
          solution.x[psi_start + n_latency], solution.x[v_start + n_latency],
          solution.x[cte_start + n_latency], solution.x[epsi_start + n_latency],
          solution.x[delta_start+n_latency],   solution.x[a_start+n_latency]};
}

void MPC::getPredictedWaypoints(vector<double> &x_out, vector<double> &y_out) {
  x_out.resize(N);
  y_out.resize(N);
  for (int i = 0; i < N; i++) {
    x_out[i] = x_predicted[i];
    y_out[i] = y_predicted[i];
  }
}
